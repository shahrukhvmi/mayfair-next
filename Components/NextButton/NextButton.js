...
const NextButton = ({ disabled, label, loading }) => {
  return (
        ...
        className={`text-white px-9 py-2 rounded-md font-medium transition-all duration-150 ease-in
                    ${
                      disabled || loading
                        ...
                        : "bg-secondary hover:bg-secondary cursor-pointer bg-violet-700"
                    }`}
      >
        {loading ? (
          ...
          // Label with Icon
          <span className="flex items-center text-sm whitespace-nowrap">{label}</span>
        )}
      </button>
    </div>
  );
